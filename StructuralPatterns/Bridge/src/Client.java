...
public class Client {
    public static void main(String[] args) {
        makeReports(new PDFImplementor());
    }
    private static void makeReports(ReportImplementor reportImplementor){
        String head = "\n\t\t\t\t\t\t\t\tRelatório Semestral de Lucros\n\n";

        String body = "\tID | Mês       | Receita (R$) | Despesas (R$) | Lucro Bruto (R$) | Lucro Líquido (R$)\n" +
                "\t-------------------------------------------------------------------------------------\n" +
        ...
        ReportFile report = new ReportFile(reportImplementor, head, body, footer);
        report.assign("Jeferson Lors");

        System.out.println(report.generate());

        DetailedReportFile detailedReportFile = new DetailedReportFile(reportImplementor, head, body, footer);
//        detailedReportFile.assign("João Victor");

        detailedReportFile.setDateTimeReport(LocalDateTime.now().toString());
        System.out.println(detailedReportFile.generate());
    }
}